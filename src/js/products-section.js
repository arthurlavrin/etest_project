...
function renderSection(
	...
	const filtered = blockFilter
		? products.filter((p) => p.blocks.includes(blockFilter))
		: products;
  ...
  `;
	...
	const cardsHTML = filtered
      ...
      <div class="product-card" data-product-id="${item.id}">
        ...
        <button class="btn add-to-cart-btn" data-product-id="${
			item.id
		}">${btnText}</button>
      </div>
    `
	...
	block.innerHTML =
		titleHTML + `<div class="products-list">${cardsHTML}</div>`;
...
function renderPagination(totalItems = filteredProducts.length) {
	...
	const prevHTML =
		currentPage > 1 ? `<button class="pagination__prev">Prev</button>` : "";
...
function applySearchAndSort() {
	...
	filteredProducts = products.filter((item) => {
		const hay =
			`${item.name} ${item.category} ${item.color} ${item.size}`.toLowerCase();
